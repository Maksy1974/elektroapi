...
exports.getById = async (req, res, next) => {
  try {
    ...
    if (!data) return res.status(404).json({ message: "Pengabdian tidak ditemukan" });
...
exports.update = async (req, res, next) => {
  try {
    const data = await prisma.pengabdian.update({
      where: { id: req.params.id },
      data: {
        judul: req.body.judul,
        topik: req.body.topik,
...
exports.remove = async (req, res, next) => {
  try {
    await prisma.pengabdian.delete({ where: { id: req.params.id } });
    return res.json({ message: "Pengabdian berhasil dihapus" });
  } catch (error) {
    return next(error);
  }
};
...
exports.addMember = async (req, res, next) => {
  try {
    const data = await prisma.anggotaPengabdian.create({
      data: {
        pengabdianId: req.params.id,
...
exports.removeMember = async (req, res, next) => {
  try {
    await prisma.anggotaPengabdian.delete({
      where: { id: Number(req.params.memberId) },
    });
    return res.json({ message: "Anggota pengabdian berhasil dihapus" });
  } catch (error) {
    return next(error);
  }
};
